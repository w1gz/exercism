...
specs = describe "encode" $ for_ cases test
  where
    ...
    test Case{..} = describe description $ do
      ...
      it "normalizes the input"    $ encode (fromString input) `shouldMatchChars` fromString expected
      it "reorders the characters" $ encode (fromString input) `shouldMatchString` fromString expected
      it "groups the output"       $ encode (fromString input) `shouldMatchWords` fromString expected
